    ...
    for _ in range(panels_series - 2):
        for col in range(0, cols, l + 1):
            for row in range(0, rows, w + 1):
                ...
                solar_patch = high_reso[row:row +
                                        (w + 1) * pw + 1, col:col + ((l * pl) + 3)]
                r, c = solar_patch.shape

                # Rotation of rectangular patch according to the angle provided
                patch_rotate = np.array(
                    [[col, row], [c + col, row], [c + col, r + row], [col, r + row]], np.int32)
                rotated_patch_points = rotation(
                    (col + c) / 2, row + r / 2, patch_rotate, solar_angle)
                ...
                if (rotated_patch_points > 0).all():
                    ...
                    polygon_points = np.array(
                        solar_polygon.exterior.coords, np.int32)
                    ...
                    if np.mean(patch_intensity_check) == 255:
                        ...
                        solar_line_1 = createLineIterator(
                            rotated_patch_points[0], rotated_patch_points[1], high_reso)
                        solar_line_1 = solar_line_1.astype(int)
                        solar_line_2 = createLineIterator(
                            rotated_patch_points[3], rotated_patch_points[2], high_reso)
                        ...
                        if len(solar_line_2) > 10 and len(solar_line_1) > 10:
                            ...
                            cv2.fillPoly(high_reso_new, [
                                         rotated_patch_points], 0)
                            cv2.polylines(high_reso_orig, [
                                          rotated_patch_points], 1, 0, 2)
                            cv2.polylines(high_reso_new, [
                                          rotated_patch_points], 1, 0, 2)

                            cv2.fillPoly(high_reso_orig, [
                                         rotated_patch_points], (0, 0, 255))
                            cv2.fillPoly(high_reso_new, [
                                         rotated_patch_points], (0, 0, 255))
                        ...
                        for points1, points2 in zip(line1_points, line2_points):
                            x1, y1, _ = points1
                            x2, y2, _ = points2
                            cv2.line(high_reso_orig, (x1, y1),
                                     (x2, y2), (0, 0, 0), 1)
                            cv2.line(high_reso_new, (x1, y1),
                                     (x2, y2), (0, 0, 0), 1)
...
if __name__ == "__main__":
    ...
    for fname in images:
        ...
        thresh = cv2.threshold(sharp_image, 0, 255,
                               cv2.THRESH_BINARY | cv2.THRESH_OTSU)[1]
        plt.figure()
        plt.title('Threshold_image')
        plt.imshow(sharp_image, cmap='gray')
        # Contours in Original Image
        contours_img(cv2.findContours(
            thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)[-2])
        # Contours in Canny Edge Image
        contours_canny(cv2.findContours(
            edged, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)[-2])
